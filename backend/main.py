...
import os
from ML.PredFunction import Text_Predict
...
# Prediction -------------------------------------------------------------------- :
@app.post('/Prediction')
async def predict_text(text: PredictionIn, MyToken: HTTPBasicCredentials = Depends(bearer_scheme)):
    email = verify_jwt(MyToken.credentials)
    db_user = get_uer(db, email)
    user_id = db_user.id

    text_p = text.user_text
    pred = Text_Predict(text_p)
...
async def get_user_predictions(
    ...
    # Ensure the connected user can only access their own predictions
    if db_user.id != id:
